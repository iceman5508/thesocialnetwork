...
visual for the new user messages
'''
...
class ComposeScreen(Screens):
    ...
    def send(self, parm):
        ...
        if to_name is None:
            ...
            if to_id is None:
                ...
                self.input_message.get_widget().text,
                "Message:",len("Message:"))
                
                if message_name is None:
                    Error(title="Messgae Error", 
                    content=Label(text="Please Proper fill the message field")).open()
                else:
                    message_interface = MessageInterface(GlobalData._user_model)
    ...
    def filter_logic(self, text, holder, width):
        """
        Filter through the information in a 
        specific text field and get useful data
