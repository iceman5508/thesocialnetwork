...
class MessageScreen(Screens):
    ...
    """
    Init all ui elements and 
    connects to business and data
    logic for messages
    ...
    def __init__(self, name):
        ...
        convo_label = Modify(Label(text = 
                                   GlobalData._user_model.get_username()+
                                    "'s Conversations"))
        convo_label.size(1, 0.2)
        
        self.new_convo = Modify(Button(text="Compose New Message", on_press=self.start_new))
    ...
    def update(self):
        """
        Update the message by
        checking across the business logic
        and update the ui upon getting a 
        new message.
        """
        try:
            if GlobalData._current_convo != None:
    ...
    def send(self, param):
        ...
        a the user currently in 
        coversation with the logged in
        user. 
        """  
        if GlobalData._current_convo != None:
            if len(self.send_message.get_widget().text) > 0:
                ...
                content=Label(text="Cannot send blank messages")).open()  
